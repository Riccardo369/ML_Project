...
#Create grid search
grid_search = GridSearch(Model, 1, grid, training_strategy)
...
#     ValidationError = 0
#     FoldsPerformance = list(
#         map(lambda fold: fold["Loss"][-1], ModelPerformance["validation"]))

#     ValidationError = sum(FoldsPerformance)/len(FoldsPerformance)
#     print(
#         f"<< finished model no.{i+1}, with a validation error of {ValidationError}")
...
# FoldsGraphVal = np.array(np.sum(list(map(
#     lambda fold: fold["Loss"], BestModelPerformance["validation"])), axis=0, dtype=np.float64))/BestModelParameters[2]
# FoldsGraphTrain = np.array(np.sum(list(map(
#     lambda fold: fold["Loss"], BestModelPerformance["training"])), axis=0, dtype=np.float64))/BestModelParameters[2]
# print(
#     f"the best model is no.{BestModelIndex} with a validation error of {BestModelError} and the following hyperparameters: {BestModelParameters}")
# del BestModelPerformance["hyperparameters"]

# Graph({"validation": FoldsGraphVal, "training": FoldsGraphTrain}, [
#       "red", "blue"], "epochs", f"test on model no. {BestModelIndex} with hyperparameters {hyperparameters}")

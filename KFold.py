...
class KFold:
    ...
    def train(self,model,batch_size,classification_function=lambda x:x):
        ...
        for i,fold in enumerate(self._folds):
            ...
            vl=DataSet(np.concatenate((*self._folds[:i+1],*self._folds[i+2:]),dtype=object),self._input_size,self._output_size,False)
            training=TrainPhase(model,tr,classification_function)
            eval_func=lambda t,o:np.sum((o-t)**2)*(1/batch_size)
            evaluation=EvaluationPhase(model,vl,eval_func,classification_function)

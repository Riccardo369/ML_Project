...
monk2_tr=TakeMonksDataSet("FilesData/monks-2.train")
...
monk2_ts=TakeMonksDataSet("FilesData/monks-2.test")
Data_ts=convert_to_one_hot(["a1","a2","a3","a4","a5","a6"],["class"],monk_encoding,monk2_ts)
...
for i in range(grid.get_size()):
    ...
    plt.title(f'MONK2,{ " ".join([ f"{k}={v} " for k,v in grid[i].items()])}')
    plt.legend()
    
    #plt.savefig(f'Plot graphic/MONK2, {" ".join([ f"{k}={v} " for k,v in grid[i].items()])} {i+1} (Loss).png')
    ...
    #plt.savefig(f'Plot graphic/MONK2, {" ".join([ f"{k}={v} " for k,v in grid[i].items()])} {i+1} (Precision).png')
    
    plt.show()
    plt.clf()
    
    print(f'MONK2, {grid[i]}')

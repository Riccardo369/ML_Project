...
class Simulator:
    def __init__(self,model,tr_dataset,vl_dataset,grid,batch_size,training_strategy,retraining_strategy=None,classification_function=lambda x:x) -> None:
        ...
        self._grid=grid
        self._grid_search=GridSearch(self._model,self._batch_size,self._grid,self._training_strategy)
    ...
    def run(self):
        ...
        if self._retraining_strategy !=None:
            retrain_result=self._retraining_strategy.train(self._model,self._batch_size,self._classification_function)
        ...
        evaluation=EvaluationPhase(self._model,self._vl_dataset,lambda t,o:np.sum((o-t)@(o-t))*(1/self._vl_dataset.size()),self._classification_function)

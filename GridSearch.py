...
class GridSearch:
    ...
    def search(self, classification_function = lambda x: x):
        ...
        for i in range(self._grid.get_size()):
            ...
            performance = self._training_strategy.train(
                self._model, self._batch_size, classification_function)
            # plot_model_performance(performance,"red","blue","epochs",f"best model with hyperparameters {self._grid[i]}")
            # evaluate performance

            # If new model is best of actual model, save it
        ...
        print(
            f"grid search done, bestmodel: np.{best_performance_index} with hyperparameters {self._grid[best_performance_index]}")
        return {
